...
function phoneMessageSendFunction() {
	...
	if (checkForm) {
		...
		if (tokenID) {
			...
			var input_phoneNumber = $('#input_phoneNumber').val();
			...
			phoneNumArr = input_phoneNumber.split(',');
			...
			for (var i = 0; i < phoneNumArr.length; i++) {
				console.log(phoneNumArr[i]);
				if(phoneNumArr[i]!==""&&phoneNumArr[i]!==null){
					phoneNumArr[i]= phoneNumArr[i].replaceAll("[- ]+", "");
					console.log('phone:'+phoneNumArr[i]);
					$
					.ajax({
						url : '/v1/messages',
						type : 'POST',
			...
			if(messageCount>0){
				alert('총 '+messageCount+"건의 메세지를 전송하였습니다.");
			}else{
...
function individualFormCheck() {
	...
	if (input_phoneNumber == null || input_phoneNumber == "") {
